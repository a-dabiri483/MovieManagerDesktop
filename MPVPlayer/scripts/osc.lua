...
--修改处
    ...
    background_color = "#0B0F19",     -- sleek dark background
    timecode_color = "#E50914",       -- accent red progress
    title_color = "#F8FAFC",          -- clean white title
    time_pos_color = "#38BDF8",       -- sky blue tooltip
    ...
    held_element_color = "#38BDF8",   -- color of an element while held down
...
local window_control_box_width = 110
local layouts = {}
...
--修改处
        ...
        wcPin = "{\\1c&H" .. osc_color_convert(user_opts.buttons_color) .. "\\fs18\\fnSegoe UI Symbol,Segoe UI,Tahoma,Arial}",
--修改处
...
--修改处
    ...
    -- Ontop / Pin: 📌
    local ne = new_element("ontop", "button")
    ne.content = function ()
        return mp.get_property_bool("ontop", false) and "📌" or "📍"
    end
    ne.eventresponder["mbtn_left_up"] =
        function ()
            local new_ontop = not mp.get_property_bool("ontop", false)
            mp.set_property_bool("ontop", new_ontop)
            mp.osd_message(new_ontop and "📌 همیشه روی صفحه: فعال" or "📍 همیشه روی صفحه: غیرفعال", 2)
            request_tick()
        end
    lo = add_layout("ontop")
    lo.geometry = alignment == "left" and fourth_geo or first_geo
    lo.style = osc_styles.wcPin
    ...
    lo.geometry = alignment == "left" and second_geo or third_geo
    lo.style = osc_styles.wcButtons
...
--box左上角y坐标 posY - pos_offsetY 但an=5 元素中心点坐标和这个坐标一致，所以还要根据an=？加减元素的高度
    ...
    -- Chapter & Navigation buttons (Hamburger button playlist_prev removed)
    lo = add_layout("chapter_prev")
    ...
    -- Move Title to Top Toolbar (Shown on mouse hover at the top)
    lo = add_layout("title")
    lo.geometry = {x = posX - 30, y = 25, an = 5, w = osc_param.playresx - 200, h = 26}
    lo.style = osc_styles.vidtitle	

    -- Always-on-top Pin button in Top Bar
    lo = add_layout("tog_ontop")
    lo.geometry = {x = osc_param.playresx - 50, y = 25, an = 5, w = 40, h = 26}
    lo.style = osc_styles.smallButtonsR

    -- Single clean row for timecodes and audio/subs in bottom bar
    lo = add_layout("tc_left")
    lo.geometry = {x = rjno1X + 10 * 20, y = rjno1Y + 10 * 4.0 + 20 / 2 + 4, an = 1, w = 110, h = 20}
    lo.style = osc_styles.vidtitle	

    lo = add_layout("cache")
    lo.geometry = {x = rjno1X + 10 * 48 + 10 * 11 / 2, y = rjno1Y + 10 * 4.0 + 4, an = 5, w = 110, h = 20}
    lo.style = osc_styles.vidtitle	

    lo = add_layout("tc_right")
    lo.geometry = {x = rjno1X + 10 * 76 + 10 * 11 / 2, y = rjno1Y + 10 * 4.0 - 20 / 2 + 4 + 4, an = 9, w = 110, h = 20}
    lo.style = osc_styles.vidtitle

    lo = add_layout("audio_track")
    lo.geometry = {x = rjno1X + 10 * 85 + 10 * 7/2, y = rjno1Y + 10 * 4.0 + 4, an = 5, w = 70, h = 20}
    lo.style = osc_styles.smallButtonsR

    lo = add_layout("sub_track")
    lo.geometry = {x = rjno1X + 10 * 93 + 10 * 7/2, y = rjno1Y + 10 * 4.0 + 4, an = 5, w = 70, h = 20}
    lo.style = osc_styles.smallButtonsR

    lo = add_layout("volume")
    lo.geometry = {x = rjno1X + 10 * 102 , y = rjno1Y + 10 * 4.0 + 4, an = 1, w = 30, h = 26}
    lo.style = osc_styles.smallButtonsR

    lo = add_layout("tog_fs")
    lo.geometry = {x = rjno1X + 10 * 106, y = rjno1Y + 10 * 4.0 + 4, an = 1, w = 30, h = 26}
    lo.style = osc_styles.smallButtonsR	
end
...
local function osc_init()
    ...
    --tog_ontop (Always on top pin button)
    ne = new_element("tog_ontop", "button")
    ne.content = function ()
        local ontop = mp.get_property_bool("ontop", false)
        if ontop then
            return "📌"
        else
            return "📍"
    ...
    ne.eventresponder["mbtn_left_up"] = function ()
        local new_ontop = not mp.get_property_bool("ontop", false)
        mp.set_property_bool("ontop", new_ontop)
        mp.osd_message(new_ontop and "📌 حالت همیشه روی صفحه فعال شد" or "📍 حالت همیشه روی صفحه غیرفعال شد", 2)
        request_tick()
    end
